...
export class NunjunksEngine implements IViewEngine {
  ...
  private get configs() {
    return this["@configs"] || (this["@configs"] = this.cfg.get(NUNJUNKS_OPTIONS));
  }

  private get loader() {
    return this["@loader"] || (this["@loader"] = new FileSystemLoader(this.configs.root, { noCache: !this.configs.cache }));
  }

  private createEnv(configs?: INunjunksRenderOptions) {
    return new Environment(this.loader, !configs ? this.configs : { ...this.configs, ...configs });
  }

  constructor(private cfg: Configs, private rs: Render) { }

  public async render(name: string, configs: any): Promise<string> {
    return await this.createEnv(configs).render(name, this.rs.views);
  }

  public async renderString(tpl: string, configs: any): Promise<string> {
    return await this.createEnv(configs).renderString(tpl, this.rs.views);
  }
